...
//        System.out.println(Arrays.toString(eqnLHSplit) + "\n" + Arrays.toString(eqnRHSplit));
        ...
        chemEqn[eqnLHSplit.length - 1] = Double.toString(eqnSolnSys[eqnLHSplit.length][eqnLHSplit.length + eqnRHSplit.length]).concat(eqnLHSplit[eqnLHSplit.length]).concat(" = ");
        System.out.println(Arrays.toString(chemEqn));
        
        for (int i = eqnLHSplit.length; i < eqnLHSplit.length + eqnRHSplit.length - 1; ++i){
            chemEqn[i] = Double.toString(eqnSolnSys[i-1][eqnLHSplit.length + eqnRHSplit.length]).concat(eqnRHSplit[i]).concat(" + ");
        }
        chemEqn[chemEqn.length - 1] = Double.toString(eqnSolnSys[eqnSolnSys.length - 1][eqnSolnSys[0].length - 1]).concat(eqnRHSplit[eqnRHSplit.length]);
